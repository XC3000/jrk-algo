...
const vehicles = require("./commArr.json"); // godigit
...
vehicles.forEach((vehicle, vehicleindex) => {
  bajaj.forEach((baj, index) => {
    vehicle.godigit["Make"] = removeSpecialCharacters(vehicle.godigit["Make"]);

    baj["vehiclemake"] = removeSpecialCharacters(baj["vehiclemake"]);
    if (
      vehicle.godigit["Make"].toLowerCase().trim() ===
        baj["vehiclemake"].toLowerCase().trim() &&
      checkModel(
        vehicle.godigit["Model"].toString().toLowerCase().trim(),
        baj["vehiclemodel"].toString().toLowerCase().trim()
      ) === "same" &&
      /* &&
      checkModel(vehicle["Model"], baj["vehiclemodel"], baj["vehiclemodel"]) ===
        "same" */
      checkVariant(vehicle.godigit["Variant"], baj["vehiclesubtype"]) === "same"
    ) {
      ...
      let commonObj = vehicles[vehicleindex];
      commonObj = {
        ...commonObj,
        bajaj: baj,
      };
